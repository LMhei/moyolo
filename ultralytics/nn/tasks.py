...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):  # 模型配置, 输入通道数, 类别数
        """
        ...
        Args:
            ...
            nc (int, optional): 检测类别数量，默认根据数据集自动推断
            verbose (bool): 详细模式，若为True则显示模型结构信息
        """
        super().__init__()       # 加载模型配置（支持直接传入字典或配置文件路径）
        ...
        if nc and nc != self.yaml["nc"]:
            ...
            self.yaml["nc"] = nc  # 覆盖YAML配置值
        # 解析模型架构（生成模型对象和保存节点列表）
        self.model, self.save = parse_model(deepcopy(self.yaml), ch=ch, verbose=verbose)  # model, savelist
        # 初始化默认类别名称字典（格式：{0: "0", 1: "1", ...}）
        ...
        # 构建特征图步长 ------------------------------------------------
        m = self.model[-1]  # 获取最后的检测头（Detect/Segment/Pose等）
        if isinstance(m, Detect):  # 包含所有检测头子类（Segment/Pose/OBB等）
            s = 256  # 基准尺寸（2倍最小步长）
            m.inplace = self.inplace        # 传递原地操作设置

            def _forward(x):
                """Perform a forward pass through the model, handling different Detect subclass types accordingly.
                   执行前向传播，处理不同检测头类型的输出差异"""
                if self.end2end:
                    return self.forward(x)["one2many"]      # 端到端模式输出
                return self.forward(x)[0] if isinstance(m, (Segment, YOLOESegment, Pose, OBB)) else self.forward(x)

            # 计算各检测层的特征图步长
            m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(1, ch, s, s))])  # 前向空张量
            self.stride = m.stride
            m.bias_init()  # 偏置初始化（仅执行一次）
        else:
            self.stride = torch.Tensor([32])  # default stride for i.e. RTDETR

        # 权重初始化 ----------------------------------------------------
        initialize_weights(self)        # 应用Kaiming初始化
        if verbose:
            self.info()
            LOGGER.info("")
